...
class DB_Map():
    ...
    def create_graph(self, path, cities):
        ...
        for city in cities:
            if type(city) == tuple:
                coordinates = self.get_coordinates(city[0])
            else:
    ...
    def return_cities(self, country, path):
        ...
        with conn:
            ...
            return cities
    def return_cities_w_pop(self, country):
        conn = sqlite3.connect(self.database)
        with conn:
            cursor = conn.cursor()
            cursor.execute('''SELECT city, population FROM cities WHERE country=?''', (country,))
            cities = [row for row in cursor.fetchall()]
            return cities
    def city_population(self, city):
        conn = sqlite3.connect(self.database)
        with conn:
            cursor = conn.cursor()
            cursor.execute('''SELECT population FROM cities WHERE city=?''', (city,))
