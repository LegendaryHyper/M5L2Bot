...
class DB_Map():
    ...
    def create_graph(self, path, cities):
        # PlateCarree harita projeksiyonu ile yeni bir grafik bağlamı oluşturma.
        # PlateCarree, boylam ve enlemlerin sırasıyla dikey ve yatay çizgiler olarak
        # gösterildiği basit bir coğrafi projeksiyondur.
        ax = plt.axes(projection=ccrs.PlateCarree())

        # Haritaya varsayılan küre görüntüsünü ekleme.
        # Bu arka plan görüntüsü, Cartopy kütüphanesi tarafından sağlanır ve
        # Dünya yüzeyini, okyanusları ve başlıca yer şekillerini içerir.
        ax.stock_img()

        # Haritada şehirleri göstermek için şehir listesi üzerinde döngü oluşturma.
        for city in cities:
    ...
    def return_cities(self, country, path):
        conn = sqlite3.connect(self.database)
        with conn:
            cursor = conn.cursor()
            # Kullanıcının tüm şehirlerini seçme
            cursor.execute('''SELECT * FROM cities WHERE country=?''', (country,))
            cities = [row for row in cursor.fetchall()]
            self.create_graph(path, cities)
            return cities 
